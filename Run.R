...
for (i in seq(as.Date("2016/10/8"), as.Date("2016/10/20"), 1)) {
...
for (i in 1:length(hpList)) {
  ...
  MinDrwDnList[[i]] <- ( MinValueList[[i]] / DataList[[i]][8]) -1
  MaxValueList[[i]] <- maxPeriod(DataList[[1]][6], hpList[[i]]*-1)
  MaxDrwDnList[[i]] <- ( MaxValueList[[i]] / DataList[[i]][8]) -1
  
  # #could consider vectorize
  TSLngList[[i]] <- data.frame(ifelse(PerctRtnLst[[i]] > plTargets[[1]] &
                                      MinDrwDnList[[i]]> -1 * plTargets[[1]]*drwDnTgt, 1, 0),
                               ifelse(PerctRtnLst[[i]] > plTargets[[2]] &
                                      MinDrwDnList[[i]]> -1 * plTargets[[2]]*drwDnTgt, 1, 0),
                               ifelse(PerctRtnLst[[i]] > plTargets[[3]] &
                                        MinDrwDnList[[i]]> -1 * plTargets[[3]]*drwDnTgt, 1, 0)
                               )
  TSShtList[[i]] <- data.frame(ifelse(PerctRtnLst[[i]] < plTargets[[1]] *-1 &
                                        MaxDrwDnList[[i]]< plTargets[[1]]*drwDnTgt, 1, 0),
                               ifelse(PerctRtnLst[[i]] < plTargets[[2]] *-1 &
                                        MaxDrwDnList[[i]]< plTargets[[2]]*drwDnTgt, 1, 0),
                               ifelse(PerctRtnLst[[i]] < plTargets[[3]] * -1 &
