...
sink("consolelog", append=FALSE, split=TRUE)
...
for (i in seq(as.Date("2016/10/8"), as.Date("2016/10/30"), 1)) {
...
for(i in 1:length(DataList)) {
  ...
  for(j in 1:ModelLag) {
    ...
    print(paste("i'm still working, variable (i) is ",i , sep = ""))
    print(paste("i'm still working, variable (j) is ", j, sep = ""))
  }
}
...
for (i in 1:length(hpList)) {
  ...
  print(paste("generating signals for holding period (i) = ", i, sep=""))
...
jpeg("graphtestBlablah.jpg")
  plot( preddfprob[3490:5000, 4] + preddfprob[3490:5000, 5]/1440, preddfprob[3490:5000, 7], type="l")
  par(new = T)
